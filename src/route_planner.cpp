...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for (RouteModel::Node* neighbor_node_ptr: current_node->neighbors) {
        neighbor_node_ptr->parent = current_node;
        neighbor_node_ptr->h_value = this->CalculateHValue(neighbor_node_ptr);
        neighbor_node_ptr->g_value = current_node->g_value + current_node->distance(*neighbor_node_ptr);

        // Set visited to true and add to open_list 
        neighbor_node_ptr->visited = true;
        this->open_list.emplace_back(neighbor_node_ptr);
    }
}
...
bool Compare(const RouteModel::Node* a, const RouteModel::Node* b) {
  auto f1 = a->g_value + a->h_value; // f1 = g1 + h1
  auto f2 = b->g_value + b->h_value;  // f2 = g2 + h2
  return f1 > f2; 
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // Start nodes parent is equal to nullptr
    while (node_ptr->parent != nullptr) {
        RouteModel::Node *parent_node_ptr = node_ptr->parent;
        distance += node_ptr->distance(*parent_node_ptr);
        path_found.emplace_back(*parent_node_ptr);
